...
import { XCircle, CheckCircle2, Copy, FileCode2, Send, ArrowRight } from 'lucide-react';
...
const STEPS = [
  { label: 'Enter UID', icon: FileCode2 },
  { label: 'Revoke', icon: Send },
...
export default function RevokePage() {
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      const tx = await service.revoke(attestationUid.trim());
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-start gap-4">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-brand-50 text-brand-500"><XCircle className="h-5 w-5" /></div>
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold tracking-tight text-surface-900">Revoke Attestation</h1>
            <span className="rounded-full bg-brand-500 px-3 py-0.5 text-xs font-medium text-white">Core Workflow</span>
          </div>
          <p className="mt-1 text-sm text-surface-500">Revoke a previously issued attestation. Only the original attester can revoke.</p>
        </div>
      </div>
        ...
        {step === 0 && (
          <div>
            <div className="max-w-lg rounded-lg border border-surface-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-surface-900">Attestation to Revoke</h2>
              <label className="mb-1.5 block text-sm text-surface-600">Attestation UID</label>
              <input type="text" placeholder="0x..." value={attestationUid} onChange={(e) => setAttestationUid(e.target.value)}
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setStep(1)} disabled={!attestationUid.trim()}
        ...
        {step === 1 && (
          <div>
            <div className="max-w-2xl rounded-lg border border-surface-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-surface-900">Confirm Revocation</h2>
              <p className="mb-4 text-sm text-surface-500">This action is irreversible. The attestation will be marked as revoked on-chain.</p>
              <div className="mb-4 rounded-md border border-surface-200 bg-surface-50 p-3">
                <p className="text-xs font-medium text-surface-500">Attestation UID</p>
                <code className="break-all text-xs text-surface-700">{attestationUid}</code>
              </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <button type="button" onClick={handleRevoke} disabled={status === 'loading' || !state.isConnected}
                className="rounded-md bg-red-500 px-6 py-2.5 text-sm font-medium text-white hover:bg-red-600 disabled:opacity-50">
                {status === 'loading' ? 'Revoking...' : 'Revoke Attestation'}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            <div className="max-w-lg rounded-lg border border-green-200 bg-green-50 p-6">
              <div className="mb-3 flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-green-600" /><h2 className="text-sm font-semibold text-green-700">Attestation Revoked</h2></div>
              <p className="mb-4 text-sm text-green-600">The attestation has been revoked on-chain.</p>
            ...
            <div className="mt-6"><button type="button" onClick={() => { setStep(0); setAttestationUid(''); setStatus('idle'); setTxHash(null); setError(null); }}
              className="rounded-md border border-surface-200 bg-white px-6 py-2.5 text-sm font-medium text-surface-600 hover:bg-surface-50">Revoke Another</button></div>
          </div>
        )}
      </FormWrapper>
    </div>
