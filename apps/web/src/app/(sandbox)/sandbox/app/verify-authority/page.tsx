...
import { ShieldCheck, CheckCircle2, Copy, FileCode2, Send, ArrowRight } from 'lucide-react';
...
const STEPS = [
  { label: 'Enter Address', icon: FileCode2 },
  { label: 'Verify', icon: Send },
...
export default function VerifyAuthorityPage() {
  ...
  const handleVerify = async () => {
    if (!contracts || !state.isConnected || !authorityAddress.trim()) return;
    setStatus('loading'); setTxHash(null); setError(null);
    try {
      const service = await contracts.getAttestationService(true);
      const tx = await service.setAuthorityVerification(authorityAddress.trim(), verified);
      ...
      <div className="mb-6 flex items-start gap-4">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold tracking-tight text-surface-900">Verify Authority</h1>
            <span className="rounded-full bg-amber-500 px-3 py-0.5 text-xs font-medium text-white">Admin Only</span>
          </div>
          <p className="mt-1 text-sm text-surface-500">
            Set verification status for a registered authority. Only the contract owner can perform this action.
          </p>
        </div>
      </div>
        ...
        {STEPS.map((s, i) => (
          <div key={s.label} className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              <div className={`flex h-8 w-8 items-center justify-center rounded-full ${i <= step ? 'bg-brand-500 text-white' : 'bg-surface-200 text-surface-400'}`}>
      ...
      <FormWrapper>
        {state.isConnected && state.address && (
          <div className="mb-6 flex items-center gap-2 rounded-md border border-green-100 bg-green-50 px-4 py-2.5">
            ...
            <button type="button" onClick={() => navigator.clipboard.writeText(state.address!)} className="ml-1 text-green-500 hover:text-green-700" aria-label="Copy address">
        ...
        {step === 0 && (
          <div>
            <div className="max-w-lg rounded-lg border border-surface-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-surface-900">Authority to Verify</h2>
              <div className="mb-4">
                <label className="mb-1.5 block text-sm text-surface-600">Authority Address</label>
                <input type="text" placeholder="0x..." value={authorityAddress} onChange={(e) => setAuthorityAddress(e.target.value)}
              ...
              <label className="flex items-center gap-2">
                ...
                <span className="text-sm text-surface-600">Mark as Verified</span>
              </label>
              <p className="mt-2 text-xs text-surface-400">Uncheck to revoke verification status</p>
            </div>
            <div className="mt-6 flex items-center justify-between">
              <span className="text-sm text-surface-400">Step 1 of 3</span>
              <button type="button" onClick={() => setStep(1)} disabled={!authorityAddress.trim()}
        ...
        {step === 1 && (
          <div>
            <div className="max-w-lg rounded-lg border border-surface-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-surface-900">Confirm Verification</h2>
              <p className="mb-4 text-sm text-surface-500">
                {verified
                  ? 'This will mark the authority as verified on-chain. Only the contract owner can do this.'
                  : 'This will remove the verified status from the authority.'}
              </p>
              <div className="mb-4 rounded-md border border-surface-200 bg-surface-50 p-3" style={{ wordBreak: 'break-all' }}>
                <p className="text-xs font-medium text-surface-500">Authority Address</p>
                <code className="text-xs text-surface-700">{authorityAddress}</code>
              </div>
              <div className="mb-4 rounded-md border border-surface-200 bg-surface-50 p-3">
                ...
                <p className="text-sm text-surface-700">{verified ? 'Set as Verified' : 'Remove Verification'}</p>
              </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <button type="button" onClick={handleVerify} disabled={status === 'loading' || !state.isConnected}
                className="rounded-md bg-brand-500 px-6 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50">
                {status === 'loading' ? 'Processing...' : verified ? 'Verify Authority' : 'Remove Verification'}
              </button>
            </div>
          </div>
        )}
            ...
            <div className="max-w-lg rounded-lg border border-green-200 bg-green-50 p-6">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-green-700">
                  {verified ? 'Authority Verified' : 'Verification Removed'}
                </h2>
              </div>
              <p className="mb-4 text-sm text-green-600">
                {verified
                  ? 'The authority has been marked as verified on-chain.'
                  : 'The authority verification has been removed.'}
              </p>
              {txHash && (
                <div className="rounded-md border border-green-200 bg-white p-3">
                  ...
                  <a href={`https://hashscan.io/testnet/transaction/${txHash}`} target="_blank" rel="noopener noreferrer"
                    className="text-xs text-brand-600 hover:underline">View on HashScan</a>
                </div>
              )}
            </div>
            <div className="mt-6">
              <button type="button" onClick={() => { setStep(0); setAuthorityAddress(''); setVerified(true); setStatus('idle'); setTxHash(null); setError(null); }}
                className="rounded-md border border-surface-200 bg-white px-6 py-2.5 text-sm font-medium text-surface-600 hover:bg-surface-50">
                Verify Another
              </button>
            </div>
          </div>
